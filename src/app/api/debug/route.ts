...
export async function GET() {
    ...
    try {
        info.ffmpegVersion = execSync('ffmpeg -version 2>&1 | head -1', { encoding: 'utf8', timeout: 5000 }).trim();
    } catch (e: any) {
    ...
    try {
        const filters = execSync('ffmpeg -filters 2>&1 | grep -i subtitle', { encoding: 'utf8', timeout: 5000 });
    ...
    try {
        ...
        if (info.fontsDirExists) {
            info.fontFiles = fs.readdirSync(fontsDir).filter(f => f.endsWith('.ttf'));
        }
    } catch (e: any) {
    ...
    // 6) System fonts
    try {
        info.systemFonts = execSync('fc-list | head -20', { encoding: 'utf8', timeout: 5000 }).trim();
    } catch (e: any) {
    ...
    try {
        ...
        fs.writeFileSync(testAss, `[Script Info]
ScriptType: v4.00+
PlayResX: 320
PlayResY: 240
...
`, 'utf8');
        ...
        // cleanup
        try { fs.unlinkSync(testAss); fs.unlinkSync(testOut); } catch { }
    } catch (e: any) {
