...
async function withRetry<T>(fn: () => Promise<T>, retries = 3): Promise<T> {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            const isTransient = ['ECONNRESET', 'ETIMEDOUT', 'ECONNABORTED', 'ERR_SOCKET_CONNECTION_TIMEOUT']
                .includes(err?.code);
            if (!isTransient || i === retries - 1) throw err;
            const delay = 1000 * Math.pow(2, i); // 1s, 2s, 4s
            console.log(`[quran-api] Retry ${i + 1}/${retries} after ${delay}ms (${err.code})`);
...
export async function getReciters(): Promise<Reciter[]> {
    const { data } = await withRetry(() => axios.get(`${BASE}/resources/recitations`));
...
export async function getAudioFiles(
    ...
    while (page <= totalPages) {
        const { data } = await withRetry(() =>
            axios.get(
                `${BASE}/recitations/${reciterId}/by_chapter/${chapter}`,
